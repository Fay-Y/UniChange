...
def main():
    ...
    model = get_model('/root/autodl-tmp/unidiffuser_joint/models/autoencoder_kl.pth')
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    model = model.to(device)

    scale_factor = 0.18215
    T = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(256), transforms.ToTensor()])
    path = '/root/autodl-tmp/unidiffuser_joint/LEVIR-MCI-dataset/images/train/label/train_000703.png'
    ...
    out = (out + 1) * 0.5
    save_image(out, f'recons_{path.split("/")[-1]}')
