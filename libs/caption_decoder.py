...
def main():
    ...
    text=['the former forest has been replaced by a residential area with roads and houses .','the former forest are removed and some new roads appear .','No change has occurred']
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # state_dict = torch.load(pretrained_path=f'caption_decoder.pth', map_location='cpu')
    # print(list(state_dict.keys())[:10])
    caption_decoder_model = CaptionDecoder(device=device, pretrained_path='./caption_decoder.pth',hidden_dim=64)
    
    import clip                         
    clip_text_model = clip.FrozenCLIPEmbedder(device=device)
    clip_text_model.to(device)
    clip_text_model.eval() # Keep in eval mode as it's a fixed encoder
    text_true_clip_dim = clip_text_model.encode(text) 
    text_true = caption_decoder_model.encode_prefix(text_true_clip_dim)

    captions = caption_decoder_model.generate_captions(text_true)
