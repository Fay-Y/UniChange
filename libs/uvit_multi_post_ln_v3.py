...
ATTENTION_MODE = 'math'  # Force using math mode for compatibility
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if ATTENTION_MODE == 'flash':
            ...
            # print('q, k, v shapes:', q.shape, k.shape, v.shape)  # Debugging output to check shapes
...
class UViT(nn.Module):
    def __init__(self, img_size, in_chans, patch_size, embed_dim=768, depth=12,
        ...
        self.diff_embed = nn.Linear(clip_img_dim, embed_dim)
        # self.diff_out = nn.Linear(embed_dim, clip_img_dim)

        self.num_text_tokens = num_text_tokens
        self.num_tokens = 1 + 1 + num_text_tokens + 1 + 3 + self.num_patches #加了diff[B, 3, 512]
    ...
    def forward(self, img, text, t_img, t_text, clip_img, diff, data_type):
        _, _, H, W = img.shape
        # print('input shape:', img.shape, clip_img.shape, text.shape, t_img.shape, t_text.shape, data_type)
        ...
        diff_tokens = self.diff_embed(diff)

        token_embed = self.token_embedding(data_type).unsqueeze(dim=1)
        # print('shape checking:', t_img_token.shape,t_text_token.shape,token_embed.shape, text.shape, clip_img.shape,diff.shape, img.shape)
        x = torch.cat((t_img_token, t_text_token, token_embed, text, clip_img, diff_tokens, img), dim=1)

        num_text_tokens, num_img_tokens = text.size(1), img.size(1)

        pos_embed = torch.cat(
            [self.pos_embed[:, :1 + 1 , :], self.pos_embed_token, self.pos_embed[:, 1 + 1 :, :]], dim=1)
        # print('pos shape',x.shape,pos_embed.shape)
        if H == self.img_size[0] and W == self.img_size[1]:
            ...
            pos_embed_others, pos_embed_patches = torch.split(pos_embed, [1 + 1 + 1 + num_text_tokens + 1 + 3, self.num_patches], dim=1)
        ...
        t_img_token_out, t_text_token_out, token_embed_out, text_out, clip_img_out, diff_out, img_out = x.split((1, 1, 1, num_text_tokens, 1, 3, num_img_tokens), dim=1)

        img_out = self.decoder_pred(img_out)
        img_out = unpatchify(img_out, self.in_chans)

        clip_img_out = self.clip_img_out(clip_img_out)
        # diff_out = self.diff_out(diff_out)
        text_out = self.text_out(text_out)

        return img_out, clip_img_out, diff, text_out
